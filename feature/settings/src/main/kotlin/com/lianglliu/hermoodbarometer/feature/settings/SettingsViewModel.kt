...
constructor(
    ...
    private val _permissionState =
        MutableStateFlow<PermissionCheckResult>(PermissionCheckResult.Idle)
    ...
    val settingsUiState: StateFlow<SettingsUiState> =
        combine(userDataRepository.userData, appLocaleManager.currentLocale) {
                userData,
                currentLocale ->
                val language =
                    availableLanguages.find { it.code == currentLocale }
                        ?: availableLanguages.first()
                SettingsUiState.Success(
                    settings =
                        UserEditableSettings(
                            colorSchemeConfig = userData.colorSchemeConfig,
        ...
        if (reminderStatus) {
            ...
            if (missingPermissions.isNotEmpty()) {
                ...
                _permissionState.value =
                    PermissionCheckResult.PermissionsRequired(missingPermissions)
                return
            }
        }
    ...
    private fun checkRequiredPermissions(): List<String> {
        ...
        // 检查通知权限
        if (
            PermissionHelpers.checkPermissionState(context, "notification") !=
