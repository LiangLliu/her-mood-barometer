...
/** 权限请求对话框 用于向用户解释为什么需要某个权限，并引导用户去设置页面 */
@Composable
fun PermissionRequestDialog(permission: String, onConfirm: () -> Unit, onDismiss: () -> Unit) {
    AlertDialog(
        onDismissRequest = onDismiss,
        title = { Text(text = getPermissionTitle(permission)) },
        text = { Text(text = getPermissionRationale(permission)) },
        confirmButton = {
